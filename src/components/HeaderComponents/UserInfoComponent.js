...
const UserInfoComponent = () => {
  ...
  useEffect(() => {
    const loadUserInfo = () => {
      ...
      const designation = localStorage.getItem('designation') || localStorage.getItem('role') || 'Employee'; // Check both designation and role
      
      console.log('🔍 Loading user info from localStorage:', { empName, empId, designation });
      ...
      console.log('🔍 UserInfo set:', newUserInfo);
    };
    
    loadUserInfo();
  }, []);

  // Debug: Track userInfo state changes
  useEffect(() => {
    ...
    <div className={styles.user_info} ref={dropdownRef}>
      ...
      <div className={styles.userinfo} onClick={toggleDropdown}>
        ...
        <div className={styles.user_details}>
          <p className={styles.username}>{userInfo.empName || 'User'}</p>
          <p className={styles.role} style={{color: 'black', fontSize: '14px'}}>
            {userInfo.designation || 'Employee'}
          </p>
          {console.log('🔍 Rendering UserInfoComponent with:', userInfo, 'isLoaded:', isLoaded, 'designation:', userInfo.designation)}
        </div>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="21"
