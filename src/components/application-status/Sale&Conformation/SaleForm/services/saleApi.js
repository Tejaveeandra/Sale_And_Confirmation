...
export const saleApi = {
  ...
  /**
   * Get orientations by class from backend
   * @param {number} classId - Class ID
   * @returns {Promise<Array>} List of orientations
   */
  async getOrientationsByClass(classId) {
    try {
      const response = await apiClient.get(`/student-admissions-sale/orientations/by-class/${classId}`);
      return response.data;
    } catch (error) {
